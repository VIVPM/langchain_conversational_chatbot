            ...
            if submit:
                if not (username and password):
                    ...
                    if user.data and hash_password(password) == user.data[0]["password"]:
                        # ---- Modified: do NOT auto-create a new chat on login if one already exists ----
                        st.session_state.logged_in = True
                        ...
                        existing_new = next(
                            (c for c in st.session_state.chats.values()
                             if c.get("title") == "New Chat" and len(c.get("messages", [])) == 0),
                            None
                        )

                        if existing_new:
                            st.session_state.selected_chat_id = existing_new["id"]
                        else:
                            if st.session_state.chats:
                                ...
                                # Create only if truly none exist
                                new_chat_id = str(uuid.uuid4())
                                ts = now_iso()
                                new_chat = {
                                    "id": new_chat_id,
...
if not st.session_state.is_processing_docs:
    ...
    if st.session_state.selected_chat_id:
        current_chat_tmp = st.session_state.chats[st.session_state.selected_chat_id]
        if current_chat_tmp["title"] == "New Chat" and len(current_chat_tmp.get("messages", [])) == 0:
    ...
    if chat_search_q:
        sorted_chats = [
            ...
            if any(chat_search_q in (m.get("content", "").lower()) for m in c.get("messages", []))
        ]
    ...
    for chat in displayed_chats:
        # ---- Modified: no date prefix for the "New Chat" row ----
                ...
                if rag_answer and "Information not found in the provided documents." not in rag_answer:
                    ...
                    direct_prompt = PromptTemplate.from_template(
                        """Answer the user's question.
                         If Conversation Facts are provided, use them only to resolve references or fill small gaps. 
                         If they are empty or irrelevant, ignore them. Do not invent details;

                         Conversation Facts (may be empty):
                         {facts_context}
                    ...
                    direct = direct_chain({"q": query, "facts_context": history_text})

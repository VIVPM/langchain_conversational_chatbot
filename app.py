import streamlit as st
import os
from config import MAX_FILES, MAX_FILE_SIZE_BYTES, DEFAULT_MODEL
from services.authentication import get_supabase, signup, login
from services.chat_manager import init_session_state, create_new_chat, persist_chats, ensure_memory_from_chat, render_history_text
from services.ai_processor import make_vectorstore, split_files, index_docs, get_llm, answer_direct, answer_from_context, web_search_answer, InputGuardrails
from ui.components import render_settings, render_web_search_toggle, render_auth_buttons, render_chat_search, render_chat_list, render_chat_history
from utils.common_utils import allow, now_iso
import warnings
from langchain_core._api.deprecation import LangChainDeprecationWarning

def init_app():
    warnings.filterwarnings("ignore", category=LangChainDeprecationWarning)
    st.title("QuickBrain")
    init_session_state(st)
    return get_supabase()

def handle_auth(supabase):
    if not st.session_state.logged_in:
        if st.session_state.show_signup:
            st.subheader("Signup")
            with st.form("signup_form"):
                u = st.text_input("Username")
                p = st.text_input("Password", type="password")
                submit = st.form_submit_button("Signup")
            if submit:
                err = signup(supabase, u, p, st)
                if err: st.error(err)
                else: st.rerun()
            if st.button("Login", key="login_acct_btn"):
                st.session_state.show_login, st.session_state.show_signup = True, False
                st.rerun()
            st.stop()

        if st.session_state.show_login:
            st.subheader("Login")
            with st.form("login_form"):
                u = st.text_input("Username")
                p = st.text_input("Password", type="password")
                submit = st.form_submit_button("Login")
            if submit:
                err = login(supabase, u, p, st)
                if err: st.error(err)
                else: st.rerun()
            if st.button("Create new account", key="create_acct_btn"):
                st.session_state.show_login, st.session_state.show_signup = False, True
                st.rerun()
            st.stop()

def render_sidebar_config():
    render_settings(st)
    api_key = st.sidebar.text_input("SambaNova API Key", type="password", disabled=st.session_state.is_processing_docs)
    model_choice = st.sidebar.selectbox("Choose model", options=[
        "Llama-3.3-Swallow-70B-Instruct-v0.4",
        "Llama-4-Maverick-17B-128E-Instruct",
        "Meta-Llama-3.1-8B-Instruct",
        "Meta-Llama-3.3-70B-Instruct",
        "DeepSeek-V3.1-Terminus",
        "DeepSeek-V3.1",
        "DeepSeek-V3-0324",
        "DeepSeek-R1-Distill-Llama-70B",
        "DeepSeek-R1-0528",
        "ALLaM-7B-Instruct-preview",
        "E5-Mistral-7B-Instruct",
        "Qwen3-32B",
        "gpt-oss-120b"
    ], index=5, disabled=st.session_state.is_processing_docs)
    tavily_api_key = st.sidebar.text_input("Tavily API Key", type="password", disabled=st.session_state.is_processing_docs)
    render_web_search_toggle(st, tavily_api_key)
    render_auth_buttons(st)
    return api_key, model_choice, tavily_api_key

def handle_file_upload(api_key, model_choice):
    uploaded_files = st.file_uploader("Upload up to 3 files each under 2MB", type=["pdf","txt","docx"], accept_multiple_files=True, disabled=st.session_state.is_processing_docs) or []
    if len(uploaded_files) > MAX_FILES:
        st.error("Select at most 3 Files.")
        st.stop()
    oversized = [uf for uf in uploaded_files if uf.size > MAX_FILE_SIZE_BYTES]
    if oversized:
        st.error("Each file must be under 1MB: " + ", ".join(f.name for f in oversized))
    else:
        if st.button("Process Documents", disabled=st.session_state.is_processing_docs or not (uploaded_files and api_key)):
            st.session_state.is_processing_docs = True
            try:
                with st.spinner("Processing documents..."):
                    vs = make_vectorstore(st.session_state.username)
                    llm = get_llm(model_choice or DEFAULT_MODEL, api_key)
                    docs = split_files(uploaded_files, llm)
                    n = index_docs(vs, docs)
                    st.session_state.vectorstore = vs
                st.success(f"Indexed {n} chunks.")
            except Exception as e:
                st.error(f"Failed to process documents: {str(e)[:150]}")
            finally:
                st.session_state.is_processing_docs = False
            st.rerun()

def render_chat_interface():
    render_chat_search(st)
    render_chat_list(st)

    if st.session_state.selected_chat_id and st.session_state.memory is None:
        # Pass LLM for summarization if API key is available
        llm_for_memory = None
        api_key = os.getenv('SAMBANOVA_API_KEY')
        if api_key:
            try:
                llm_for_memory = get_llm(DEFAULT_MODEL, api_key)
            except ConnectionError as e:
                st.sidebar.warning(f"Memory LLM connection failed: {str(e)[:50]}")
            except ValueError as e:
                st.sidebar.warning(f"Invalid API key for memory: {str(e)[:50]}")
            except Exception as e:
                # Log unexpected errors for debugging
                st.sidebar.caption(f"Memory LLM unavailable: {type(e).__name__}")
        st.session_state.memory = ensure_memory_from_chat(
            st.session_state.chats[st.session_state.selected_chat_id], 
            llm=llm_for_memory
        )
    if st.session_state.selected_chat_id:
        render_chat_history(st.session_state.chats[st.session_state.selected_chat_id])

def handle_chat_interaction(api_key, model_choice, tavily_api_key, supabase):
    if not st.session_state.is_processing_docs:
        q = st.chat_input("Your question")
        if q:
            if st.session_state.selected_chat_id is None:
                create_new_chat(st)
                
            is_valid, err = InputGuardrails.check_toxicity_and_explicit(q)
            final, source_block = None, ""
            llm = get_llm(model_choice or DEFAULT_MODEL, api_key)
            cur = st.session_state.chats[st.session_state.selected_chat_id]
            t = now_iso()
            if cur["title"] == "New Chat":
                cur["title"] = q[:50] + ("..." if len(q) > 50 else "")
                st.session_state["_sidebar_title_just_updated"] = True

            cur["messages"].append({"role": "user", "content": q, "date": t})
            with st.chat_message("user"): st.markdown(q)

            with st.spinner("Generating answer..."):
                if not allow("llm_calls", limit=50, window_sec=60):
                    st.warning("Rate limit: 50 requests per minute.")
                    st.stop()
                if not err:
                    history_text = render_history_text(st.session_state.memory)

                    if st.session_state.use_web_search and tavily_api_key:
                        final, sources = web_search_answer(llm, tavily_api_key, q)
                        source_block = "\n\n**Web Sources used:**\n" + "\n".join(f"- {s}" for s in sources) if sources else ""
                    else:
                        if "vectorstore" in st.session_state:
                            retriever = st.session_state.vectorstore.as_retriever(search_kwargs={"k": 10}, search_type="mmr")
                            docs = retriever.get_relevant_documents(q)
                            context = "\n\n".join(d.page_content for d in docs)
                            rag = answer_from_context(llm, context, q)
                            if rag and "Information not found in the provided documents." not in rag:
                                final = rag
                                uniq_src = sorted({d.metadata.get("source","") for d in docs if d.metadata.get("source")})
                                if uniq_src:
                                    source_block = "\n\n**Sources used:**\n" + "\n".join(f"- {s}" for s in uniq_src)
                        if not final:
                            final = answer_direct(llm, history_text, q)
                else:
                    final = answer_direct(llm, err, q)
                    
                assistant_response = f"{final}{source_block}"            
            if not is_valid:
                assistant_response += f"\n\n⚠️ {err}"
            cur["messages"].append({"role": "assistant", "content": assistant_response, "date": t})
            cur["updated_at"] = now_iso()
            
            st.session_state.memory.save_context(
                {"input": q},                 
                {"answer": assistant_response}  
            )
                
            with st.chat_message("assistant"):
                st.markdown(assistant_response)
                
            persist_chats(supabase, st.session_state.user_id, st)
            if st.session_state.pop("_sidebar_title_just_updated", False):
                st.rerun()

def main():
    supabase = init_app()
    
    handle_auth(supabase)

    api_key, model_choice, tavily_api_key = render_sidebar_config()
    handle_file_upload(api_key, model_choice)
    render_chat_interface()
    
    if api_key:
        handle_chat_interaction(api_key, model_choice, tavily_api_key, supabase)
    else:
        st.info("Enter your SambaNova API key in the sidebar.")

if __name__ == "__main__":
    main()
